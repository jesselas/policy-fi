...
2. **X/Twitter (normal page fetch returns HTTP 402):** use the free, no-login syndication endpoint, which works from the cloud sandbox — `curl -sS -A "Mozilla/5.0" "https://cdn.syndication.twimg.com/tweet-result?id=TWEET_ID&lang=en&token=a"`. Extract the numeric `TWEET_ID` from the status URL. The JSON returns the full tweet `text`, `user.name`/`screen_name`, `created_at`, and `entities.urls[].expanded_url` (the real resource the post links to). Then fetch that `expanded_url` (via step 1 if it is Cloudflare-gated) and evaluate the underlying resource — the tweet is usually just a pointer. `r.jina.ai` does NOT work for x.com (it blocks the domain anonymously); use the syndication endpoint instead.
